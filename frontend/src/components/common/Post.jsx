...
const Post = ({ post }) => {
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    mutationFn: async () => {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Something went wrong");
      return data; // updated likes array from server
    },
    onSuccess: (updatedLikes) => {
      // Update query cache after server confirms
      queryClient.setQueryData(["posts"], (oldData = []) =>
        oldData.map((p) =>
          p._id === post._id ? { ...p, likes: updatedLikes } : p
        )
      );
      // Refetch posts to sync everything
      queryClient.invalidateQueries({ queryKey: ["posts"] });
    },
  ...
  // Use the query cache for current post data so UI updates live
  const currentPost =
    queryClient.getQueryData(["posts"])?.find((p) => p._id === post._id) || post;
  const isLiked = authUser ? currentPost.likes.includes(authUser._id) : false;
        ...
        <div className="flex justify-between mt-3">
          <div className="flex gap-4 items-center w-2/3 justify-between">
            ...
            <div className="flex gap-1 items-center cursor-pointer group" onClick={() => document.getElementById("comments_modal" + post._id).showModal()}>
              ...
              <span className="text-sm text-slate-500 group-hover:text-sky-400">{currentPost.comments.length}</span>
            </div>
      ...
      <dialog id={`comments_modal${post._id}`} className="modal border-none outline-none">
        <div className="modal-box rounded border border-gray-600">
          ...
          <div className="flex flex-col gap-3 max-h-60 overflow-auto">
            {currentPost.comments.length === 0 && <p className="text-sm text-slate-500">No comments yet 🤔 Be the first one 😉</p>}
